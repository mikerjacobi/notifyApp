...
func (s *NotifyAppServer) insertJournal(ctx context.Context, db Database, j *pb.Journal) error {
	stmt, err := db.Prepare(`
		INSERT INTO journals (journal_id, comms_id, phone_number, prompt, entry, created, updated)
		VALUES (?, ?, ?, ?, ?, NOW(6), NOW(6))
	`)
	if err != nil {
	...
	if _, err = stmt.Exec(j.JournalId, j.CommsId, j.PhoneNumber, j.Prompt, j.Entry); err != nil {
...
func (s *NotifyAppServer) getJournalEntries(ctx context.Context, db Database, phoneNumber string) ([]*pb.Journal, error) {
	stmt, err := db.Prepare(`SELECT journal_id,comms_id,phone_number,prompt,entry,created,updated 
		FROM journals 
		WHERE phone_number=?
	...
	for rows.Next() {
		j := &pb.Journal{}
		if err := rows.Scan(&j.JournalId, &j.CommsId, &j.PhoneNumber, &j.Prompt, &j.Entry, &j.Created, &j.Updated); err != nil {
